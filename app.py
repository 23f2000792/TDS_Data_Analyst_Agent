...
'''
    ...
    script_lines.append(inspect.getsource(scrape_url_to_dataframe))
...
You will receive:
- A set of **rules**
- A **task description** (questions and required JSON keys)
- An optional **dataset preview**

You must:
1. Follow rules exactly.
2. Return only a valid JSON object with a single key: `code`.
3. The value of `code` must be Python that populates a dict called `results`.
4. Do NOT import scrape_url_to_dataframe (already available).
5. Use plot_to_base64() for plots.
6. Numeric values in `results` must be numbers, not strings.
"""),
    ("human", "{input}"),
...
agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True, max_iterations=5, early_stopping_method="generate", handle_parsing_errors=lambda e: f"Output parsing error: {e}", return_intermediate_steps=False)
...
async def analyze_data(request: Request):
    try:
        ...
        questions_file = None; data_file = None
        ...
        pickle_path = None; df_preview = ""; dataset_uploaded = False
        if data_file:
            ...
            temp_pkl = tempfile.NamedTemporaryFile(suffix=".pkl", delete=False); temp_pkl.close()
            df.to_pickle(temp_pkl.name); pickle_path = temp_pkl.name

            df_preview = f"\n\nDataset Preview:\nColumns: {', '.join(df.columns.astype(str))}\nFirst 5 rows:\n{df.head(5).to_markdown(index=False)}\n"

        if dataset_uploaded:
            llm_rules = "Rules:\n1) Use the provided pandas DataFrame `df`.\n2) DO NOT call scrape_url_to_dataframe().\n"
        else:
            llm_rules = "Rules:\n1) You must call scrape_url_to_dataframe(url).\n"

        llm_input = f"{llm_rules}\nTask Description:\n{raw_questions}\n{df_preview if df_preview else ''}Respond with the JSON object only."
        ...
        if not code:
            raise HTTPException(500, detail=f"Invalid agent response: {parsed}")
        ...
        for key in keys_list:
            if key in results_dict:
                ...
                try:
                    ...
                    if isinstance(val, str) and caster != str and not val.startswith(('iVBO', '/9j/')):
