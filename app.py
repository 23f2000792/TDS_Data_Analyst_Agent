...
def parse_keys_and_types(raw_questions: str) -> Tuple[List[str], Dict[str, Any]]:
    """
...
def clean_llm_output(output: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if first == -1 or last == -1 or last <= first:
            return {"error": "No JSON object found in LLM output", "raw": s[:500]}

        candidate = s[first:last + 1]
        try:
            return json.loads(candidate)
        except Exception as e:
            # Try shortening tail until it parses
            for i in range(last, first, -1):
                ...
                try:
                    ...
                    continue
            return {"error": f"JSON parsing failed: {str(e)}", "raw": candidate[:500]}
    except Exception as e:
        return {"error": str(e)}
...
def write_and_run_temp_python(code: str, injected_pickle: str = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    Write a temp python file which:
      ...
      - defines a robust plot_to_base64() helper that ensures < 100kB
...
2) Return ONLY a valid JSON object — no extra commentary or formatting, no markdown fences.
...
4) Your Python code will run in a sandbox with:
   ...
   - helper `plot_to_base64(max_bytes=100000)` for base64-encoded plots
5) When returning plots, always use plot_to_base64().
6) Ensure all variables are defined before use. The code must run without undefined references.
7) Do NOT include triple backticks anywhere.
""",
        ),
        ("human", "{input}"),
...
# -----------------------------
def run_agent_safely(llm_input: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.debug(f"Raw LLM output: {raw_out!r}")
        ...
        if not isinstance(parsed, dict):
            return {"error": f"Parsed output is not a JSON object", "raw": str(parsed)[:500]}

        if "code" not in parsed or "questions" not in parsed:
            return {"error": f"Expected keys 'code' and 'questions' not found.", "raw": parsed}
    ...
    try:
        ...
        if result and isinstance(result, dict):
            if keys_list and type_map:
                ...
                for idx, (q, val) in enumerate(items):
                    if idx < len(keys_list):
                        ...
                        try:
                            ...
                            mapped[key] = caster(val) if (val not in (None, "") and caster is not str) else val
                        except Exception:
                            mapped[key] = val
                result = mapped
...
# Favicon (1x1 transparent PNG fallback)
...
def _env_check(required=None):
    ...
    for k in required:
        v = os.getenv(k)
        out[k] = {"present": bool(v), "masked": (v[:4] + "..." + v[-4:]) if v else None}
